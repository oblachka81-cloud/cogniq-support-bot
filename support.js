...
// Приветствие с фото и одной кнопкой
bot.start(async (ctx) => {
  ...
  await ctx.replyWithPhoto(
    { source: './support_avatar.png' },
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: t.support_btn, web_app: { url: 'https://cogniqai.bothost.tech' } }]
        ]
      }
    }
  );
...
// Приём языка из WebApp
bot.on('web_app_data', async (ctx) => {
  try {
    const data = JSON.parse(ctx.webAppData.data);
    if (data.action === 'set_lang' && data.lang) {
      await saveUserLang(ctx.from.id, data.lang);
    }
  } catch(e) {}
});
