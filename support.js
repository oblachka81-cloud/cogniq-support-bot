...
// Проверка Telegram-подписи (как в основном проекте)
  ...
  if (OPENROUTER_API_KEY) {
    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'nvidia/nemotron-3-ultra-550b-a55b:free', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: question }] })
      });
  ...
  if (apiKey && folderId) {
    try {
      const response = await fetch('https://ai.api.cloud.yandex.net/v1/responses', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Api-Key ${apiKey}`, 'OpenAI-Project': folderId },
        body: JSON.stringify({ model: `gpt://${folderId}/yandexgpt-5-lite/latest`, instructions: systemPrompt, input: question, temperature: mode === 'chat' ? 0.7 : 0.2, max_output_tokens: 500 })
      });
      const data = await response.json();
...
bot.start(async (ctx) => {
  ...
  let langCode = 'en', strings = LANG['en'];
  try {
    const { rows } = await pool.query('SELECT lang FROM user_langs WHERE user_id = $1', [userId]);
    if (rows[0]?.lang && LANG[rows[0].lang]) { langCode = rows[0].lang; strings = LANG[rows[0].lang]; }
  } catch(e) {}
...
// Страница-заглушка для неавторизованных
...
http.createServer(async (req, res) => {
  ...
  if (req.url.startsWith('/api/user-info') && req.method === 'GET') {
    ...
    try {
      ...
      await pool.query(`INSERT INTO user_avatars (user_id, first_name, tg_photo_file_id, updated_at) VALUES ($1, $2, $3, NOW()) ON CONFLICT (user_id) DO UPDATE SET first_name = $2, tg_photo_file_id = $3, updated_at = NOW()`, [userId, firstName, fileId]);
      let avatarUrl = null;
    ...
    req.on('end', async () => {
      try {
        ...
        if (data.user_id && data.message) {
          ...
          if (SUPPORT_OWNER_ID) bot.telegram.sendMessage(SUPPORT_OWNER_ID, `📩 Новый тикет от ${data.user_id}:\n\n${data.message}`);
        }
        res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  // Статические файлы
  if (req.url === '/support_avatar.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_avatar.png'))); return; }
  if (req.url === '/support_bg.webp') { res.writeHead(200, { 'Content-Type': 'image/webp' }); res.end(fs.readFileSync(path.join(__dirname, 'support_bg.webp'))); return; }
  if (req.url === '/support_lang_ru.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_lang_ru.png'))); return; }
  if (req.url === '/support_lang_en.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_lang_en.png'))); return; }
  if (req.url === '/support_lang_fr.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_lang_fr.png'))); return; }
  if (req.url === '/support_lang_es.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_lang_es.png'))); return; }
  if (req.url === '/support_faq_btn.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_faq_btn.png'))); return; }
  if (req.url === '/support_mode_support.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_mode_support.png'))); return; }
  if (req.url === '/support_mode_chat.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_mode_chat.png'))); return; }
  if (req.url === '/support_back_btn.png') { res.writeHead(200, { 'Content-Type': 'image/png' }); res.end(fs.readFileSync(path.join(__dirname, 'support_back_btn.png'))); return; }
